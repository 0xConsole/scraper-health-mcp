...
from app.heal_orchestrator import orch, HealOutcome
from app.health_checker import HealthStatus
from app.mcp_server import MCP_TOOLS, handle_tool_call, get_mcp_manifest
...
async def api_demo():
    ...
    demo_results["steps"].append(f"🤖 Triggered self-heal → AI proposed diff")
    demo_results["steps"].append(f"✅ Auto-approved via resume_automation_job → healed!")
    demo_results["steps"].append(f"🔄 Re-scraped → health restored")
...
async def api_reset():
    ...
    global orch
    from app.heal_orchestrator import HealOrchestrator
    orch = HealOrchestrator()
    if orch.client.mock_mode:
        from app.heal_orchestrator import seed_demo_collectors
